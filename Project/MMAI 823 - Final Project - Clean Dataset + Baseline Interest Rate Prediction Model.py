...
#Dropping columns 
data_df.drop(['dti_joint', 'verification_status_joint', 'annual_inc_joint', 'il_util', 
'mths_since_rcnt_il', 'all_util', 'max_bal_bc'],axis=1,inplace=True) 
...
data_df.drop(['loan_status', 'verification_status', 'sub_grade', 'emp_length', 'home_ownership','term'],axis=1,inplace=True)

print('Data rows:',data_df.shape[0], 'Columns:',data_df.shape[1])

#Replace NaN values in open_rv_24m column with 0
data_df['open_rv_24m'] = data_df['open_rv_24m'].fillna(0)

#Replace NaN values in open_rv_12m column with 0
data_df['open_rv_12m'] = data_df['open_rv_12m'].fillna(0)

#Replace NaN values in total_cu_tl column with 0
data_df['total_cu_tl'] = data_df['total_cu_tl'].fillna(0)

#Replace NaN values in total_bal_il column with 0
data_df['total_bal_il'] = data_df['total_bal_il'].fillna(0)

#Replace NaN values in open_il_24m column with 0
data_df['open_il_24m'] = data_df['open_il_24m'].fillna(0)

#Replace NaN values in open_il_12m column with 0
data_df['open_il_12m'] = data_df['open_il_12m'].fillna(0)

#Replace NaN values in open_il_6m column with 0
data_df['open_il_6m'] = data_df['open_il_6m'].fillna(0)

#Replace NaN values in open_acc_6m column with 0
data_df['open_acc_6m'] = data_df['open_acc_6m'].fillna(0)

#Replace NaN values in inq_fi column with 0
data_df['inq_fi'] = data_df['inq_fi'].fillna(0)

#Replace NaN values in inq_last_12m column with 0
data_df['inq_last_12m'] = data_df['inq_last_12m'].fillna(0)

#Replace NaN values in mths_since_last_record column with 0
data_df['mths_since_last_record'] = data_df['mths_since_last_record'].fillna(0)
...
x = data_df.drop(['int_rate', 'grade'], axis=1).values 
